...
- **Consequence Editor** — `renderConsequences()` expandable `<details>` cards with inline editing (type dropdown, location input, text textarea). `addConsequence()`, `updConsequence(i,k,v)`, `remConsequence(i)`, `clearResolvedConsequences()` (bulk delete resolved with confirmation). Enlarged resolve button, aria-labels on resolve/delete buttons. `+` button in section header.
